...
class HybridEncryptedData:
    ...
    def decrypt(self, private_key_path):
        ...
        decrypted_bytes = unpad(cipher.decrypt(encrypted_data[16:]), 16)
        
        # Convert the decrypted byte data to a string
        decrypted_data_str = decrypted_bytes.decode('utf-8')

        # Convert the decrypted data (string) to a dictionary if it's in JSON format
        try:
            decrypted_data_json = json.loads(decrypted_data_str)
            self.decrypted_data = decrypted_data_json
            return DecryptedData(self.decrypted_data)
        except json.JSONDecodeError:
            print("Decrypted data is not in JSON format.")
            return decrypted_data_str  # Return the raw string if not in JSON format
